...
public class PlayerWithRaycastControl : NetworkBehaviour
{
    ...
    [SerializeField]
    private NetworkVariable<PlayerControl.PlayerState> networkPlayerState = new NetworkVariable<PlayerControl.PlayerState>();
    ...
    private PlayerControl.PlayerState oldPlayerState = PlayerControl.PlayerState.Idle;
    ...
    private void FixedUpdate()
    {
        if (IsClient && IsOwner)
        {
            if (networkPlayerState.Value == PlayerControl.PlayerState.Punch && ActivePunchActionKey())
            {
    ...
    private void ClientVisuals()
    {
        if (oldPlayerState != networkPlayerState.Value)
        {
            ...
            if (networkPlayerState.Value == PlayerControl.PlayerState.Punch)
            {
    ...
    private void ClientInput()
    {
        ...
        {
            UpdatePlayerStateServerRpc(PlayerControl.PlayerState.Punch);
            return;
        }

        // change motion states
        if (forwardInput == 0)
            UpdatePlayerStateServerRpc(PlayerControl.PlayerState.Idle);
        else if (!ActiveRunningActionKey() && forwardInput > 0 && forwardInput <= 1)
            UpdatePlayerStateServerRpc(PlayerControl.PlayerState.Walk);
        else if (ActiveRunningActionKey() && forwardInput > 0 && forwardInput <= 1)
        {
            inputPosition = direction * runSpeedOffset;
            UpdatePlayerStateServerRpc(PlayerControl.PlayerState.Run);
        }
        else if (forwardInput < 0)
            UpdatePlayerStateServerRpc(PlayerControl.PlayerState.ReverseWalk);
    ...
    [ServerRpc]
    public void UpdatePlayerStateServerRpc(PlayerControl.PlayerState state)
    {
        networkPlayerState.Value = state;
        if (state == PlayerControl.PlayerState.Punch)
        {
